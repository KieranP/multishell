...
| [docs/design/state-and-store.md](docs/design/state-and-store.md) | Why persisted state is repaired rather than trusted, why projects are strict where the rest are lossy, why invariants are tested at random, when a file is moved aside. |
...
| [docs/design/worktrees.md](docs/design/worktrees.md) | What is watched and what is polled, naming, the trunk row and row ordering, removal to the Trash, a branch name refused before git runs, bare repositories, a missing directory, the NUL-terminated list. |
