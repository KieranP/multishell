...
struct TabDragSourceGoneTests {
  ...
  private func host(
    ...
    button: PrimaryButton = PrimaryButton(heldDown: false)
  ) -> NSWindow {
    let host = NSHostingView(
      rootView: TabGroupsView(model: h.model, worktree: worktree, theme: .multishellDark)
        .environment(\.primaryButton, button))
  ...
  @Test func aDragWhoseStripStopsScrollingStaysInTheAirWhileTheButtonIsDown() {
    ...
    let window = host(
      ...
      button: PrimaryButton(heldDown: true))
    h.model.beginTabDrag(tabs[2])
