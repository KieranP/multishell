...
struct SessionStatesTests {
  ...
  @Test func failedSurvivesALookAndTheProcessThatFailed() {
    var states = SessionStates()
    states.report(.failed, pid: 7, for: .session(a), isSeen: true)
    ...
    states.report(.failed, pid: 9, for: .session(b), isSeen: false)
  ...
  @Test func aNonZeroExitIsFailedAndOutlivesAnUnseenDone() {
    ...
    states.report(.failed, pid: nil, for: .worktree("/w"), isSeen: false)
