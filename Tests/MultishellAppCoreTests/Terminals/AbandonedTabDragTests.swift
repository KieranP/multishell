...
struct AbandonedTabDragTests {
  ...
  @Test func aDropWithNoTabInTheAirIsRefused() throws {
    let h = Harness()
    let tabs = threeTabs(h)
    let column = try #require(h.model.workspace.tab(tabs[0])?.groupID)

    #expect(!h.model.dropDraggedTab(on: .area(column)))
    #expect(!h.model.dropDraggedTab(on: .strip(column)))
    #expect(!h.model.dropDraggedTab(on: .tab(tabs[1], .after)))
    #expect(!h.model.dropDraggedTab(on: .band(.init(groupID: column, placement: .after))))
    #expect(order(h) == tabs)
  }
  ...
  @Test func aTabDroppedBesideATabClosedBeforeTheMoveRunsGoesBack() async {
    ...
    let columns = h.model.workspace.groups(in: h.main.id).map(\.id)
    let first = h.model.workspace.tabs(in: columns[0]).map(\.id)
    let anchor = h.model.workspace.tabs(in: columns[1]).map(\.id)[0]
    ...
    #expect(h.model.workspace.tabs(in: columns[0]).map(\.id) == first)
  }

  @Test func aTabDroppedOnItsOwnColumnsTerminalAreaGoesBackWhereTheDragFoundIt() async throws {
    let h = Harness()
    let tabs = threeTabs(h)
    let column = try #require(h.model.workspace.tab(tabs[2])?.groupID)
    h.model.beginTabDrag(tabs[2])
    h.model.shuffleTab(tabs[2], .before, past: tabs[0])
  ...
  @Test func aTabDroppedOnAnotherColumnsTerminalAreaJoinsIt() async throws {
    let h = Harness()
    let tabs = threeTabs(h)
    h.model.moveActiveTabToNewGroup()
  ...
  @Test func aBandThatRefusesTheTabLeavesItWhereItWas() async throws {
    ...
    let column = try #require(h.model.workspace.tab(tab)?.groupID)
