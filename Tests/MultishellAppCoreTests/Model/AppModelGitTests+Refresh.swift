...
extension AppModelGitTests {
  ...
  @Test func aCreateWithNoStagesAfterItLetsGoOfTheRow() async throws {
    ...
    #expect(h.model.workInFlight.claimedPaths.isEmpty)
    try "x".write(
  ...
  @Test func twoCreatesAtOnceEachHoldTheirOwnRow() async throws {
    ...
    for _ in 0..<250 where h.model.workInFlight.claimedPaths.count < 2 {
      try await Task.sleep(for: .milliseconds(20))
    }
    let held = h.model.workInFlight.claimedPaths
    ...
    #expect(h.model.workInFlight.claimedPaths.isEmpty, "and each let go of its own")
