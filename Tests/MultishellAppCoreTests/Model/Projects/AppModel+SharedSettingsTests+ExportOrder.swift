...
extension AppModelSharedSettingsTests {
  @Test func aPollReadingTheFileBeforeExportFinishesAsksNothing() async throws {
    ...
    let written = AppModel<FakeSurface>.writeSharedSettings(export)
  ...
  @Test func anExportWrittenLateNeverLandsOverOneAskedAfterIt() async throws {
    ...
    h.model.finishSharedSettingsExport(second, AppModel<FakeSurface>.writeSharedSettings(second))
    h.model.finishSharedSettingsExport(first, AppModel<FakeSurface>.writeSharedSettings(first))
  ...
  @Test func anExportFinishingLateLeavesTheLaterOneRecorded() async throws {
    ...
    let firstWritten = AppModel<FakeSurface>.writeSharedSettings(first)
    h.model.finishSharedSettingsExport(second, AppModel<FakeSurface>.writeSharedSettings(second))
