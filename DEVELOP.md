...
## Layout
    ...
    Sources/
      ...
      MultishellProcess/        ProcessRunner, ShellCommand, ExecutableLookup
    ...
    Apps/macOS/                 SwiftUI app; its own Package.swift
      Sources/Multishell/
        ...
        Terminals/              hosts (Ghostty, SwiftTerm, MultiEngine),
                                SurfaceView, PaneTreeView + WeightedSplit, TabBar
      ...
      Tests/MultishellTests/    the app's pure parts: error mapping, metrics,
...
- Every field of every persisted type decodes with a default. State from an
  older build must load. `PersistenceTests` and `DecodingDefaultsTests` cover
  the shapes so far; add a case there when you add a field.
...
- Parsers of git output (`WorktreeListParser`, `WorktreeStatusParser`) are
  pure functions tested on fixture text, including CRLF.
...
- `state.json`: projects, worktrees, tabs, pane trees, appearance, engine,
  worktree defaults. Not processes; each saved tab gets a fresh shell.
...
model-facing code. Linux and Windows have never been compiled locally; CI is
