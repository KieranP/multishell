...
- `WorktreeWorkInFlight`'s create handle and `worktreeCreationStep` are one slot
  each, so with two creates overlapping, Cmd+N reopening the sheet while the
  last one runs, the first to end nils both: the second sheet's Cancel then does
  nothing, its hook and its `git worktree add` cannot be stopped, and its later
  steps are dropped as belonging to a create that has ended. The row hold-back
  is counted per create and is not affected.
...
- A split or column divider drag now holds its weights in the view and writes
  ...
  190pt at 10-point to 346pt at 18, and a width of zero or NaN reads as "no
