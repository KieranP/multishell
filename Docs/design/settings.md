...
Export writes the settings in force back over the repository's file, and a hook
the user refused is not in force -> exporting would have dropped a teammate's
committed hook silently. So export keeps the file's own hooks where the user
wrote none, and records trust only where every hook written is the user's own
words; a refused hook stays refused against the new digest, so nothing asks
again about a decision already made.
