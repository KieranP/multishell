...
@Suite
struct QuitMessageTests {
  @Test func workingAgentsAreCountedApartFromShells() {
    #expect(AppDelegate.quitMessage(terminals: 1, working: 0).hasPrefix("One terminal"))
    #expect(
      AppDelegate.quitMessage(terminals: 3, working: 1)
        == "3 terminals are still open and will be closed. One of them has an agent that is still working."
    )
    #expect(
      AppDelegate.quitMessage(terminals: 4, working: 2).hasSuffix(
