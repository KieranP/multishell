...
extension AppModel {
  ...
  private func notifyIfNeeded(
    ...
    guard
      NotificationPolicy.shouldNotify(
        ...
        appIsActive: NSApp?.isActive ?? true, duration: report.duration),
    ...
    notifier.notify(
      title: "\(subject) · \(project) › \(worktree.name)",
  ...
  func state(of tab: TerminalTab) -> SessionState? {
    sessionStates.state(ofSessions: tab.sessionIDs)
  }

  func state(ofWorktree id: Worktree.ID) -> SessionState? {
  ...
  func state(ofProject id: Project.ID) -> SessionState? {
  ...
  var workingAgentCount: Int { sessionStates.workingSessionCount }

  func clearState(of tab: TerminalTab) {
    mutateStates { $0.clear(sessions: tab.sessionIDs, worktree: nil) }
    updatePIDWatch()
  }

  func clearState(ofWorktree id: Worktree.ID) {
  ...
  func updatePIDWatch() {
    ...
    pidWatch = Task { @MainActor [weak self] in
      while !Task.isCancelled {
        ...
        for pid in sessionStates.trackedPIDs where Self.processIsGone(pid) {
  ...
  /// Only ESRCH means gone; EPERM is another user's live process.
  nonisolated static func processIsGone(_ pid: Int32) -> Bool {
