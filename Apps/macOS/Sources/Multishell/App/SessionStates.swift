...
///
/// Runtime only, beside `liveSessions`; a prompt must not save or re-render
/// the workspace. Done is about the user and clears when the tab is shown.
/// Working and Waiting are about the process: they stay while the user
/// looks, and clear when the source reports again, the process is gone, or
/// the user clears them by hand. Pure, so the rules are tested without a
/// view or an engine.
struct SessionStates: Equatable {
  enum Key: Hashable {
  ...
  private(set) var states: [Key: SessionState] = [:]
  /// The process behind a Working or Waiting state, when the report said.
  private(set) var pids: [Key: Int32] = [:]

  subscript(key: Key) -> SessionState? { states[key] }

  // MARK: - Sources

  /// A report over the channel. `isShown` means the user is looking at the
  /// tab, or at the worktree for a worktree-level report.
  mutating func report(_ state: SessionState, pid: Int32?, for key: Key, isShown: Bool) {
    switch state {
    case .idle:
      clear(key)
    case .done, .error:
  ...
  mutating func noteActivity(in id: TerminalSession.ID, isShown: Bool) {
  ...
  mutating func noteCommandFinished(in id: TerminalSession.ID, exitCode: Int32?, isShown: Bool) {
    let key = Key.session(id)
  ...
  var trackedPIDs: Set<Int32> { Set(pids.values) }
