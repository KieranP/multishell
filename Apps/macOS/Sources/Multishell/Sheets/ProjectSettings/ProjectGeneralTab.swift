    ...
    Form {
      ...
      Section {
        HStack(spacing: 8) {
          ...
          InfoButton(
            "Writes this project's worktree path, branch prefix, hooks, the files new worktrees are given, icon, what its worktrees open and the order they are listed in, as they are in effect, to \(SharedProjectSettings.fileName) at the repository root, for the team to commit, replacing one already there. Anyone who adds the repository gets them as defaults under their own; they are asked once before its hooks run."
          )
          Spacer()
