...
/// `DirectoryWatcher` on kqueue via `DispatchSource`.
///
/// A directory source fires when its direct children change, not deeper, so
/// callers pass every level they care about. Events are coalesced for 400 ms
/// because git touches several files per operation.
@MainActor
final class DispatchDirectoryWatcher: DirectoryWatcher {
