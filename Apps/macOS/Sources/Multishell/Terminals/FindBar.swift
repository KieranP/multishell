...
struct FindBar: View {
  ...
  var body: some View {
    ...
    HStack(spacing: 4) {
      ...
      step("xmark", t("find.close")) { model.hideFind(of: sessionID) }
    }
    .padding(Self.inset)
  ...
  private func well(_ metrics: UIMetrics) -> some View {
    TextField(t("find.prompt"), text: needle)
      ...
      .onSubmit(submit)
      .onExitCommand { model.hideFind(of: sessionID) }
      .padding(.horizontal, 10)
      .frame(height: (metrics.body * 2.2).rounded())
  ...
  private func step(_ symbol: String, _ label: String, action: @escaping () -> Void) -> some View {
    StepButton(symbol: symbol, label: label, theme: theme, size: model.metrics.body, action: action)
  }
}
  ...
  var body: some View {
    Button(action: action) {
      Image(systemName: symbol)
        ...
        .frame(width: (size * 2.2).rounded(), height: (size * 2.2).rounded())
