...
that the card's own lines are a fixed three: what a card said about itself used
to be buried under a title that had taken a second line and a badge that had
taken a line of its own, and the eye could not find it twice in the same place.
The message block below them is still one to three lines and still optional, so
