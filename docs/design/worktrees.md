...
and what is made cannot part. `creationStopper` and `worktreeCreationStep` are
still single slots, so under that overlap the second create's Cancel is lost;
...
front, one `git status` every five seconds. `StatusPollPace` remembers how long
