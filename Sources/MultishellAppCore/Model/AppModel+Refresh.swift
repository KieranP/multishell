...
extension AppModel {
  ...
  public func refresh(_ project: Project) async {
    ...
    do {
      let discovered = try await worktrees.git.list(project)
      guard isStillListedAfterGit(project.id) else { return }
  ...
  private func isStillListedAfterGit(_ id: Project.ID) -> Bool {
  ...
  func commonGitDirectory(of project: Project) async -> URL? {
    ...
    guard let worktrees, let common = try? await worktrees.git.commonGitDirectory(project) else {
      return nil
    }
  ...
  public func fetch(_ project: Project) async {
    ...
    do {
      try await worktrees.git.fetch(project)
    } catch {
      report(error)
      return
    }
