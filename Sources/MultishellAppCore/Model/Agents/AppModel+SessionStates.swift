...
extension AppModel {
  ...
  func pruneStates() {
    ...
    mutateStates { $0.retain(sessions: liveSessionIDs, worktrees: worktrees) }
  }
  ...
  /// is no agent, one typed at the prompt with no hooks installed is.
  func isAgentWorking(in id: TerminalSession.ID) -> Bool {
    sessionStates.isAgentWorking(in: id, commandIsAgent: commandAgentIDs[id] != nil)
  }
