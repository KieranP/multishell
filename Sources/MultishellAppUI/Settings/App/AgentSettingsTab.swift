...
struct AgentSettingsTab: View {
  ...
  private var agentSection: some View {
    Section {
      ...
      if model.workspace.preferredAgentID == AgentCatalogue.customID {
        InfoLabeledContent(t("agents.command"), info: t("agents.command-info")) {
          TextField(
            t("agents.command"),
    ...
    Section(t("agents.hooks-section")) {
      ForEach(model.agentHooksRows) { row in
        InfoLabeledContent(t("agents.row-label", row.name), info: row.info) {
          ...
          Button(t("agents.show", row.contentsName)) { shownSnippetRowID = row.id }
            .popover(isPresented: isShowing(row), arrowEdge: .bottom) { snippetPopover(for: row) }
        }
        .controlSize(.small)
      }
    }
  ...
  private var pathSourceInfo: String {
