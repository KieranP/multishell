...
struct ColumnDropBands: View {
  ...
  var body: some View {
    GeometryReader { proxy in
      // A column too narrow to halve offers no band, rather than make two
      // columns nobody can read; the release reaches the area beneath.
      ...
      ZStack {
        ...
        Color.clear
          ...
          .onDrop(
            ...
            delegate: ColumnAreaDropDelegate(
              groupID: group.id,
              drag: $drag,
  ...
  private func band(_ placement: TerminalTab.Placement) -> some View {
    ...
    return Color.accentColor
      ...
      .onDrop(
        of: [TabTransfer.contentType],
        delegate: ColumnBandDropDelegate(
          band: target,
          drag: $drag,
