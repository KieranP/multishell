  ...
  var body: some View {
    face
      ...
      .onDragSessionUpdated { session in
        if case .ended = session.phase { model.endAbandonedTabDrag(tab.id) }
      }
      // The session's end reaches only a source still on screen.
      .onDisappear {
        model.tabDragSourceLeft(tab.id, isPressed: { [primaryButton] in primaryButton.isDown })
      }
      .onDrop(
...
struct PrimaryButton: Equatable, Sendable {
  var heldDown: Bool?
...
extension EnvironmentValues {
  @Entry var primaryButton = PrimaryButton()
}
