...
public struct SharedSettingsRead: Equatable, Sendable {
  ...
  public init(
    ...
    stamp: Date = .distantPast, problem: String? = nil, hasBeenRead: Bool = false
  ) {
  ...
  /// for that, `stamp` included: `hasMoved` says yes to it.
  public static let unread = SharedSettingsRead()
  ...
  public func hasMoved(_ stamp: Date) -> Bool { self.stamp != stamp || !hasBeenRead }
}

extension SharedSettingsRead {
  /// A read that parsed, confined by the reader, which asks the disk. The
  /// stamp is recorded either way, so an unparsable file is not re-read.
  public mutating func note(
    _ settings: SharedProjectSettings?, confined: SharedProjectSettings?, stamp: Date
  ) {
    asWritten = settings
    self.confined = confined
    self.stamp = stamp
  ...
  public mutating func note(problem: String, stamp: Date) {
    asWritten = nil
    confined = nil
    self.stamp = stamp
    self.problem = problem
