...
public enum SessionEnvironment {
  ...
  public static func variables(for session: TerminalSession, socket: URL) -> [String: String] {
    ...
    variables.merge(zshIntegration()) { current, _ in current }
    return variables
  }

  /// When the app has generated the zsh integration directory and the user's
  ...
  static func zshIntegration(
    ...
    let shell = URL(fileURLWithPath: environment["SHELL"] ?? "").lastPathComponent
    guard shell == "zsh",
