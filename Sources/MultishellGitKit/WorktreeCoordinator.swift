...
public struct WorktreeCoordinator: Sendable {
  ...
  public func plannedPath(
    forBranch branch: String, in project: Project, settings: WorktreeSettings
  ) -> URL {
    settings.worktreePath(forBranch: settings.qualifiedBranch(branch), in: project)
  }
  ...
  public func create(
    ...
    let branch = settings.qualifiedBranch(rawBranch)
